...
class SetupMainWindow:
    ...
    add_left_menus = [
        ...
        },
        {
            ...
            "btn_id": "btn_Check",
            "btn_text": "Virus Total",
            "btn_tooltip": "Virus Total",
            "show_top": True,
            "is_active": True
        },
    ...
    def setup_gui(self):
        ...
        # 홈 페이지에 버튼 추가
        self.logo_label = QLabel(self.ui.load_pages.page_1)
        self.logo_pixmap = QPixmap("gui_2/gui/images/svg_images/logo.png")  # 로고 이미지 경로
        self.logo_pixmap = self.logo_pixmap.scaled(500, 500, Qt.KeepAspectRatio,
                                                   Qt.SmoothTransformation)  # 로고 이미지 크기 조정
        self.logo_label.setPixmap(self.logo_pixmap)
        self.logo_label.setAlignment(Qt.AlignCenter)  # 가운데 정렬

        # QVBoxLayout을 생성합니다.
        layout = QVBoxLayout(self.ui.load_pages.page_1)
        layout.addWidget(self.logo_label, 0, Qt.AlignCenter)  # 로고를 레이아웃 중앙에 추가
        layout.addStretch()  # 로고와 버튼 사이에 여백 추가

        self.homeButton = QPushButton("KISA Check", self.ui.load_pages.page_1)
        self.homeButton.setMinimumHeight(40)
        # 버튼 스타일 설정 (이미 정의되어 있음)

        # 버튼 크기 설정
        button_width = 380  # 버튼의 너비, 필요에 따라 조정
        button_height = 100  # 버튼의 높이, 필요에 따라 조정
        self.homeButton.setFixedSize(button_width, button_height)

        layout.addWidget(self.homeButton, 0, Qt.AlignCenter)  # 버튼을 레이아웃 중앙에 추가
